...
module.exports = {
  ...
  PageMutation: {
     ...
     * REORDER PAGE TREE
     */
    async reorderTree (obj, args) {
      try {
        if (!Array.isArray(args.order) || args.order.length < 1) {
          throw new Error('No tree items specified.')
        }
        const locale = args.locale || WIKI.config.lang.code
        const parentId = args.parent && args.parent > 0 ? args.parent : null
        const desiredOrder = args.order.map(id => _.toInteger(id))
        if (_.some(desiredOrder, id => id < 1)) {
          throw new Error('Invalid tree items specified.')
        }
        if (_.uniq(desiredOrder).length !== desiredOrder.length) {
          throw new Error('Duplicate tree items are not allowed.')
        }
        const nodes = await WIKI.models.knex('pageTree')
          .select('id', 'localeCode', 'parent')
          .whereIn('id', desiredOrder)
        if (nodes.length !== desiredOrder.length) {
          throw new Error('One or more tree items could not be found.')
        }
        const invalidNode = _.find(nodes, node => node.localeCode !== locale || ((node.parent || null) !== parentId))
        if (invalidNode) {
          throw new Error('Tree items do not belong to the requested directory.')
        }
        await WIKI.models.knex.transaction(async trx => {
          for (const [idx, nodeId] of desiredOrder.entries()) {
            await trx('pageTree')
              .where({ id: nodeId })
        ...
        return {
          responseResult: graphHelper.generateSuccess('Directory order updated successfully.')
        }
      } catch (err) {
        return graphHelper.generateError(err)
      }
